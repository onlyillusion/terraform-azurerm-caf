...
module "postgresql_flexible_servers" {
  ...
  remote_objects = {
    ...
    private_dns_zone_id = can(each.value.private_dns_zone.key) ? local.combined_objects_private_dns[try(each.value.private_dns_zone.lz_key, local.client_config.landingzone_key)][each.value.private_dns_zone.key].id : try(each.value.private_dns_zone_id, null)
